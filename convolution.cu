...
#define RESULT_VERIFICATION 1   

__global__ void convolution_kernel_version_1(
    float *d_output, 
    float *d_input, 
    float *d_filter, 
    int num_row, 
...
__global__ void convolution_kernel_version_2(
    float *d_output, 
    float *d_input, 
    float *d_filter, 
    int num_row, 
...
__global__ void convolution_kernel_version_3(
    float *d_output, 
    float *d_input, 
    float *d_filter, 
    int num_row, 
...
)
{
    ...
    for (int row = 0; row <= tile_size / BLOCK_DIM; row++)
    {
        ...
        {
            int idx_row = idx_y + BLOCK_DIM * row - pad_size;   // input data from index row
            int idx_col = idx_x + BLOCK_DIM * col - pad_size;   // input data from index column
...
)
{
    ...
    if (version == 1)
        convolution_kernel_version_1<<<dimGrid, dimBlock>>>(d_output, d_input, d_filter, num_row, num_col, filter_size);
    else if (version == 2) 
        convolution_kernel_version_2<<<dimGrid, dimBlock>>>(d_output, d_input, d_filter, num_row, num_col, filter_size);
    else // version == 3
    {
        int shared_mem_size = (2*filter_size+BLOCK_DIM) * (2*filter_size+BLOCK_DIM) * sizeof(float);
        convolution_kernel_version_3<<<dimGrid, dimBlock, shared_mem_size, 0 >>>(d_output, d_input, d_filter, num_row, num_col, filter_size);
    }
    
    checkCudaErrors(cudaGetLastError());
}

void convolution_host(
    float *h_output, 
    float *h_input, 
    float *h_filter, 
    int num_row, 
